...
```txt
The core idea is to directly place each element
to its new position after left rotation
using the index mapping (i + d) % n.
```
...
int* rotateLeft(int d, int arr_count, int* arr, int* result_count) {
    *result_count = arr_count;
    ...
    for (int i = 0; i < arr_count; ++i) result[i] = arr[(i + d) % arr_count];
    return result;
}
